...
import com.perspectia.perspectiabackend.config.PasswordEncoder;
import com.perspectia.perspectiabackend.email.EmailUtility;
import com.perspectia.perspectiabackend.exceptions.EmailNotVerifiedException;
import com.perspectia.perspectiabackend.exceptions.RefreshTokenExpiredException;
import com.perspectia.perspectiabackend.exceptions.UserAlreadyExistsByEmailException;
import com.perspectia.perspectiabackend.exceptions.UserNotFoundException;
import com.perspectia.perspectiabackend.jwt.JwtHelper;
import com.perspectia.perspectiabackend.models.Role;
import com.perspectia.perspectiabackend.models.User;
import com.perspectia.perspectiabackend.repositories.RoleRepository;
import com.perspectia.perspectiabackend.repositories.UserRepository;
import com.perspectia.perspectiabackend.requests.*;
import com.perspectia.perspectiabackend.responses.LoginResponse;
import com.perspectia.perspectiabackend.responses.RefreshTokenResponse;
import com.perspectia.perspectiabackend.responses.UserProfileResponse;
...
public class AuthenticationService {
    ...
    public LoginResponse login(LoginRequest loginRequest, HttpServletResponse response) {
        ...
        User user = userRepository.findByEmail(loginRequest.getEmail()).orElseThrow(
                () -> new UserNotFoundException("User Not Found Exception with Email:" + loginRequest.getEmail()));
        if (!user.isVerified()) {
        ...
        ResponseCookie refreshCookie = ResponseCookie.from("refreshToken", refreshToken).httpOnly(true).secure(true)
                .path("/perspectia/auth/token/refresh").maxAge(7 * 24 * 60 * 60).sameSite("None").build();
    ...
    public String resendOtp(ResendOtpRequest resendOtpRequest) {
        ...
        user.setOtpGeneratedTime(LocalDateTime.now());
        userRepository.save(user);
        emailUtility.sendEmail(user.getEmail(), "Otp Verification for ProJam", "Otp for Verification is:" + newOtp);
    ...
    public RefreshTokenResponse generateRefreshToken(RefreshTokenRequest refreshTokenRequest) {
        Optional<User> optUser = userRepository.findByEmail(refreshTokenRequest.getEmail());
        if (optUser.isEmpty()) {
            throw new UserNotFoundException("User Not Found with the Gmail Provided.");
        }
        User user = optUser.get();
        if (user.getEmail().equals(refreshTokenRequest.getEmail()) && user.isVerified()
